(function( $ ){
	...
	var defaults = {
		...
		'close' : '.close_it',
		'reverse' : '.reverse_it',
		'marker' : 'images/pins/centro.png',
		'airport' : 'images/pins/airport.png',
		'art_gallery' : 'images/pins/artgallery.png',
		'bakery' : 'images/pins/bakery.png',
		'bank' : 'images/pins/bank.png',
		'bar' : 'images/pins/bar.png',
		'cafe' : 'images/pins/cafe.png',
		'church' : 'images/pins/church.png',
		'gym' : 'images/pins/gym.png',
		'hospital' : 'images/pins/hospital.png',
		'museum' : 'images/pins/museum.png',
		'night_club' : 'images/pins/night_club.png',
		'pharmacy' : 'images/pins/pharmacy.png',
		'restaurant' : 'images/pins/restaurant.png',
		'school' : 'images/pins/school.png',
		'taxi_stand' : 'images/pins/taxi.png',
		'university' : 'images/pins/university.png',
		'train_station' : 'images/pins/train2.png',
		'subway_station' : 'images/pins/marker_subway_station.png',
	...
	var methods = {
		...
		init : function( options ) {			
			...
			if (!pattern.test(window[plugin][pi].bakery) ) { window[plugin][pi].bakery = window[plugin][pi].hostname + window[plugin][pi].bakery; }			
			if (!pattern.test(window[plugin][pi].bank) ) { window[plugin][pi].bank = window[plugin][pi].hostname + window[plugin][pi].bank; }			
			if (!pattern.test(window[plugin][pi].bar) ) { window[plugin][pi].bar = window[plugin][pi].hostname + window[plugin][pi].bar; }			
			if (!pattern.test(window[plugin][pi].cafe) ) { window[plugin][pi].cafe = window[plugin][pi].hostname + window[plugin][pi].cafe; }
			if (!pattern.test(window[plugin][pi].church) ) { window[plugin][pi].church = window[plugin][pi].hostname + window[plugin][pi].church; }
			if (!pattern.test(window[plugin][pi].gym) ) { window[plugin][pi].gym = window[plugin][pi].hostname + window[plugin][pi].gym; }
			if (!pattern.test(window[plugin][pi].hospital) ) { window[plugin][pi].hospital = window[plugin][pi].hostname + window[plugin][pi].hospital; }
			if (!pattern.test(window[plugin][pi].museum) ) { window[plugin][pi].museum = window[plugin][pi].hostname + window[plugin][pi].museum; }
			if (!pattern.test(window[plugin][pi].night_club) ) { window[plugin][pi].night_club = window[plugin][pi].hostname + window[plugin][pi].night_club; }
			if (!pattern.test(window[plugin][pi].pharmacy) ) { window[plugin][pi].pharmacy = window[plugin][pi].hostname + window[plugin][pi].pharmacy; }
			...
			if (!pattern.test(window[plugin][pi].school) ) { window[plugin][pi].school = window[plugin][pi].hostname + window[plugin][pi].school; }
			...
			if (!pattern.test(window[plugin][pi].university) ) { window[plugin][pi].university = window[plugin][pi].hostname + window[plugin][pi].university; }
				...
				if(window[plugin][pi].group == true) {
					...
					if(window[plugin][pi].pois != null) {
						...
						$.map($.makeArray(window[plugin][pi].pois), function(value, n) {							
							var _poi = $('<'+ _elem.get(0).tagName +'><input type="radio" name="place_poi_'+ pi +'" id="place_poi_'+ pi +'_'+ n +'"><label for="place_poi_'+ pi +'_'+ n +'">'+ value.name +'</label></'+ _elem.get(0).tagName +'>');
							...
							if(window[plugin][pi].display_pois == false) {
								$('input', _poi).on('change', function() {
									...
									google.maps.event.addListener(window[plugin][pi].markerPOI, 'click', (function(plugin, value) {
										return function() {
											plugin.poiInfowindow.setContent(value.content+'<a href="javascript:;" onclick="$.fn.FBMap(\'poi\',\''+ pi +'\',\''+value.lat+','+value.lng+'\')" class="place_itinerary">'+plugin.link_text+'</a>');
								...
								window[plugin][pi].markerPOI[n] = new google.maps.Marker({
									map: window[plugin][pi].mapCanvas,
									position: new google.maps.LatLng(value.lat,value.lng)
								});
								google.maps.event.addListener(window[plugin][pi].markerPOI[n], 'click', (function(plugin, value) {
									return function() {
										plugin.poiInfowindow.setContent(value.content+'<a href="javascript:;" onclick="$.fn.FBMap(\'poi\',\''+ pi +'\',\''+value.lat+','+value.lng+'\')" class="place_itinerary">'+ plugin.link_text +'</a>');
										plugin.poiInfowindow.open(plugin.mapCanvas, this);								
									}
								})(window[plugin][pi], value));
								window[plugin][pi].bounds.extend(window[plugin][pi].markerPOI[n].position);
								$('input', _poi).on('change', function() {
									google.maps.event.trigger(window[plugin][pi].markerPOI[n], 'click');
								});
							}
							
							if(value.category !== undefined) {
								if($.inArray(value.category,_categories) == -1) { _categories.push(value.category); _categories[value.category] = new Array(); }
						...
						if(_categories.length > 0) {
							_categories.map(function(val, cat) {
								var _category = $('<'+ _elem.get(0).tagName +'><strong>'+ val +'</strong><'+ $(window[plugin][pi].monuments).get(0).tagName +'></'+ $(window[plugin][pi].monuments).get(0).tagName +'></'+ _elem.get(0).tagName +'>');
				...
				google.maps.event.addListenerOnce(window[plugin][pi].mapCanvas, 'idle', function(){					
					...
					if(window[plugin][pi].pois == null) {
						$(window[plugin][pi].monuments).find(window[plugin][pi].selectors).on('click', function() {
							...
							if(window[plugin][pi].is_checked == _item) { $(this).get(0).checked = false; } else { window[plugin][pi].is_checked = _item; }
							methods.places(pi);
						});
					}					
						...
						service.nearbySearch(request, function(results, status) {
							...
							if (status != google.maps.places.PlacesServiceStatus.OK) {
								...
								for (var i = 0, _rl = results.length; i < _rl; i++) {
									...
									var _obj = Object.keys(results[i].geometry.location);
									
									results[i].placeItinerary = $('<a href="javascript:;" onclick="$.fn.FBMap(\'poi\',\''+ pi +'\',\''+results[i].geometry.location[_obj[0]]+','+results[i].geometry.location[_obj[1]]+'\',\''+ _type +'\')" class="place_itinerary">'+ window[plugin][pi].link_text +'</a>');
									
									results[i].placeContent = results[i].image + '<div style="float:left; width:150px;" class="infobox"><strong>' + results[i].name + '</strong><br>' + results[i].vicinity +'</div><br><br>' + results[i].placeItinerary[0].outerHTML;
		...
		calcRoute : function(pi, start, type, poi, end) {
			...
			if( window[plugin][pi] !== undefined ) {
				...
				$(window[plugin][pi].itinerary).removeClass('walking');
				
				if(start == undefined || typeof(start) == 'object' || type == 'airport') {
					if(type != 'airport') { var start = $(window[plugin][pi].itineraryInput).val(); }
					var travelMode = google.maps.DirectionsTravelMode.DRIVING;
				} else {
					var travelMode = google.maps.DirectionsTravelMode.WALKING;
					$(window[plugin][pi].itinerary).addClass('walking');
				}
				...
				window[plugin][pi].directionsService.route(request, function(response, status) {					
					if (status == google.maps.DirectionsStatus.OK) {
						if(window[plugin][pi].group == true) {
							for(var n in window[plugin][pi].hotels) { if(window[plugin][pi].hotels[n].infowindow) { window[plugin][pi].hotels[n].infowindow.close(); } }
						} else {
