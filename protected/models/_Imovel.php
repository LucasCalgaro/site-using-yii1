...
//    private $codImob = 179;
    ...
    public function getCriterioBuscaRapida($post) {
        ...
        $finalidade = Yii::app()->user->hasState('finalidade') 
                    ? Yii::app()->user->getState('finalidade') 
                    : ( isset($post['finalidade']) 
                            ? $post['finalidade'] 
                            : null );
        $estado     = Yii::app()->user->hasState('estado') 
                    ? Yii::app()->user->getState('estado') 
                    : ( isset($post['estado']) 
                            ? $post['estado'] 
                            : null );
        $cidade     = Yii::app()->user->hasState('cidade') 
                    ? Yii::app()->user->getState('cidade') 
                    : ( isset($post['cidade']) 
                            ? $post['cidade'] 
                            : null );
        $bairro     = Yii::app()->user->hasState('bairro') 
                    ? Yii::app()->user->getState('bairro') 
                    : ( isset($post['bairro']) 
                            ? $post['bairro'] 
                            : null );
        $tipo       = Yii::app()->user->hasState('tipo') 
                    ? Yii::app()->user->getState('tipo') 
                    : ( isset($post['tipo']) 
                            ? $post['tipo'] 
                            : null );
        $referencia = Yii::app()->user->hasState('referencia') 
                    ? Yii::app()->user->getState('referencia') 
                    : ( isset($post['referencia']) 
                            ? $post['referencia'] 
                            : null );
        $valor_min  = Yii::app()->user->hasState('valor_min') 
                    ? Yii::app()->user->getState('valor_min') 
                    : ( isset($post['valor_min']) 
                            ? $post['valor_min'] 
                            : null );
        $valor_max  = Yii::app()->user->hasState('valor_max') 
                    ? Yii::app()->user->getState('valor_max') 
                    : ( isset($post['valor_max']) 
                            ? $post['valor_max'] 
                            : null );
    ...
    public function trataBenfeitoria($string) 
    {
        $criteria = new CDbCriteria;
        ...
        foreach ($ids as $id) {
            ...
            if (isset($return[$id[0]])) {
                $item = $return[$id[0]];
                $benfeitorias .= self::TrataTipoBenfeitoria($item, $tp, $qtd);
            }
        }
        return $benfeitorias;
    }
