...
?>
echo scUtil::getTituloDaJanela($this, 'Visualizar <?= $aLegenda ?>');

/*
$this->beginWidget('zii.widgets.CPortlet', array(
       'title'=>'<h3>Visualiza <?php echo $this->modelClass." ' . \$model->{$this->tableSchema->primaryKey} . '"; ?></h3>',
...
<?php
   foreach($this->tableSchema->columns as $column) {
       if(stristr($column->dbType, "decimal") )
	echo "\t\tarray('name'=>'" . $column->name . "', 'value'=>money_format('%.2n', \$model->" . $column->name . ")),\n";
       else
	echo "\t\t'".$column->name."',\n";
   }
?>
